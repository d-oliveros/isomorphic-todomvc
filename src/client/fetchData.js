 ...
 */
export default function fetchData(routerState, tree) {
  debug(`Fetching component data. Branch is:\n${inspect(routerState.branch)}`);

  let components = routerState.branch.map(route => route.component);
  let promises = fetchComponentsData(components, routerState, tree);
...
function fetchComponentsData(components, routerState, tree) {
  ...
  let componentsFetchData = components
